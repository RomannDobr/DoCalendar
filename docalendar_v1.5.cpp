...
void nowData();
void newSeason(int, string);
void userCount(int, string);
int charToInt(char);
void autorun(int);
void manual();
...
int main()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (firstfiles.is_open())
        {
            ...
            { // поиск стартовой метки события - "|"
                firstfiles >> buf;
                if (buf == '|')
                {
                    firstfiles >> buffer1; // считывание названия из одного слова
                    for (int i = 0; buf != '.'; i++)
                    { // считывание большого названия
                        firstfiles >> buf;
                        if (buf == '.')
                            break;
    ...
    if (mesyac >= 9)
    { // условие для запуска счётчика до нового года
        oneoftvelve = 1;
        oneofday = 0;
        oneyear = 100;
        struct tm c = {0, 0, 0, oneofday, oneoftvelve - 1, oneyear + 2, 0, 0, 0};
        time_t newYear = mktime(&c);
        newSeason(newYear, "  До нового года");
    }
    if (mesyac <= 2)
    { // условие для запуска счётчика до весны
        oneoftvelve = 3;
        oneofday = 0;
        oneyear = 100;
        struct tm c = {0, 0, 0, oneofday, oneoftvelve - 1, oneyear + 1, 0, 0, 0};
        time_t spring = mktime(&c);
        newSeason(spring, "  До весны");
    }
    if (mesyac >= 3 && mesyac < 6)
    { // условие для запуска счётчика до лета
        oneoftvelve = 6;
        oneofday = 0;
        oneyear = 100;
        struct tm c = {0, 0, 0, oneofday, oneoftvelve - 1, oneyear + 1, 0, 0, 0};
        time_t summer = mktime(&c);
        newSeason(summer, "  До лета");
    }
    if (mesyac >= 6 && mesyac < 9)
    { // условие для запуска счётчика до конца лета
        oneoftvelve = 9;
        oneofday = 0;
        oneyear = 100;
        struct tm c = {0, 0, 0, oneofday, oneoftvelve - 1, oneyear + 1, 0, 0, 0};
        time_t fall = mktime(&c);
        newSeason(fall, "  До конца лета");
    }
    cout << "\n\n";
    ...
    if (efiles.is_open())
    {
        ...
        for (int i = 0; i < *n; i++)
        {
            efiles >> buf;
            if (buf == '|')
            { // персчитывает колличество событий. Для отображения
                j++;
                if (j == 1)
                {
                    efiles >> event1;
    ...
    if (j < 4)
        cout << "  Создать событие? (нажмите 1)\n\n";
    if (j > 0)
        cout << "  Удалить событие? ";
    ...
    LONG check = RegGetValueA(HKEY_CURRENT_USER,
                              "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run", "DoCalendar", RRF_RT_REG_SZ, 0, 0, 0);
    if (check == 0)
        cout << "  Удалить из автозагрузки (8)\n";
    if (check == 2)
        cout << "  Добавить в автозагрузку (9)\n";
    ...
    if (question >= 3 && question <= 6)
    {
        ...
        for (int i = 0, l = 0; i <= o; i++)
        {
            if (buff[i] == '|')
                l++;
            if (question == 3 && l != 1 && l != 0)
                delfiles1 << buff[i];
            if (question == 4 && l != 2 && l != 0)
                delfiles1 << buff[i];
            if (question == 5 && l != 3 && l != 0)
                delfiles1 << buff[i];
            if (question == 6 && l != 4 && l != 0)
                delfiles1 << buff[i];
        }
        SetConsoleOutputCP(1251); // SetConsoleOutputCP(1251);
        SetConsoleCP(1251);       //         SetConsoleCP(1251);
...
void newSeason(int y, string season)
{ // счётчик до НГ, весны, лета, конца лета
    if (x != (time_t)(-1) && y != (time_t)(-1))
    {
        ...
        cout << season;
        cout << " " << difference << " дн. ";
        if ((difference / 7) % 10 == 1)
            cout << difference / 7 << " полная нед." << endl;
        if ((difference / 7) == 0)
            cout << "Mеньше нед." << endl;
        if ((difference / 7) % 10 != 1 && (difference / 7) != 0)
            cout << difference / 7 << " полных нед." << endl;
        cout << " ___________________________________________________________________/\n";
    }
}

void userCount(int y, string message)
...
{ // преодразование char в int (от 0 до 9)
    char character[10];
    int x;
    for (int i = 0; i < 10; i++)
    {
...
void autorun(int tog)
{
    ...
    else if (tog == 9)
    {
        ...
        if (ERROR_SUCCESS == key)
            key = RegSetValueExA(hkey, "DoCalendar", 0, REG_SZ,
...
{
    cout << "\n\n -------------------------------------------------------\n";
    cout << " DoCALENDAR — консольная утилита для отслеживания дней\n";
    cout << " до или после важных событий, праздников и личных дат.\n\n";
    cout << " Возможности:\n";
    cout << "  - Отображение текущей даты и дня недели.\n";
    cout << "  - Подсчёт дней до/после личных событий (до 4 штук).\n";
    cout << "  - Подсчёт дней до Нового года, весны, лета, конца лета.\n";
    cout << "  - Добавление/удаление событий.\n";
    cout << "  - Автозагрузка программы при старте Windows.\n\n";
    cout << " Системные требования:\n";
    cout << "  - Операционная система: Windows.\n";
    cout << "  - 10 МБ свободного места на диске.\n\n";
    cout << " Установка:\n";
    cout << "  1. Скачайте файл `DoCalendar.exe`.\n";
    cout << "  2. Запустите программу.\n";
    cout << "  3. Следуйте инструкциям в консоли.\n\n";
    cout << " Использование:\n";
    cout << "  - Для создания нового события нажмине 1.\n";
    cout << "  - Для удаления события выберите его номер (3-6).\n";
    cout << "  - Для добавления/удаления из автозагрузки нажмите 8/9.\n";
    cout << "  - Для просмотра инструкции нажмите 2.\n\n";
    cout << " Особенности:\n";
    cout << "  - Русскоязычный интерфейс.\n";
    cout << "  - Автоматическое сохранение данных.\n\n";
    cout << " Безопасность:\n";
    cout << "  - Программа не требует интернет-соединения.\n";
    cout << "  - Не отправляет данные.\n\n";
    cout << " Файлы:\n";
    cout << "  - `DoCalendar_data.txt` — файл с данными.\n";
    cout << "  - `DoCalendar.exe` — исполняемый файл программы.\n";
    cout << " -------------------------------------------------------";
}