...
void newSeason(int x, int y, string a) { // счётчик до НГ, весны, лета, конца лета
    if(x != (time_t)(-1) && y != (time_t)(-1)) {
        ...
        cout << a;
        cout << " " << difference << " дн. ";
        if((difference / 7) % 10 == 1) cout << difference / 7 << " полная нед." << endl;
        if((difference / 7) == 0) cout << "Mеньше нед." << endl;
        if((difference / 7) % 10 != 1 && (difference / 7) != 0) cout << difference / 7 << " полных нед." << endl;
        cout << " ___________________________________________________________________/\n";
    }
}

void userCount(int x, int y, string message) { // отображение счётчиков
    fstream name;
    char msg;
    char re[MAX_PATH];
    string FP = string(re, GetModuleFileNameA(NULL, re, MAX_PATH));
